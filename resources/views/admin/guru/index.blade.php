                ...
                <tbody class="divide-y divide-gray-100" id="tableBody">
                    @forelse($guru as $index => $g)
                        <tr class="hover:bg-gray-50 transition guru-row"
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0
                                        {{ strtolower($g->jenis_kelamin ?? '') === 'perempuan' ? 'bg-pink-100' : 'bg-primary-100' }}">
                                        <span class="font-bold text-xs
                                            {{ strtolower($g->jenis_kelamin ?? '') === 'perempuan' ? 'text-pink-600' : 'text-primary-700' }}">
                            ...
                            <td class="px-5 py-3.5 text-gray-600">{{ $g->email }}</td>

                            <td class="px-5 py-3.5">
                                @if(strtolower($g->jenis_kelamin ?? '') === 'perempuan')
    ...
    <script>
        ...
        function filterTable() {
            ...
            document.querySelectorAll('.guru-row').forEach(row => {
                const ok = (row.dataset.nama.includes(search) || row.dataset.email.includes(search))
