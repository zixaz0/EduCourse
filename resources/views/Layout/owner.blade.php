        ...
        <header
            ...
            <div class="flex items-center gap-3">
                <div class="text-right hidden sm:block">
                    ...
                    <p class="text-sm font-semibold">{{ Auth::user()->name ?? (Auth::user()->username ?? 'Owner') }}</p>
                </div>
                <div
                    class="w-9 h-9 rounded-full border-2 border-yellow-300 flex items-center justify-center bg-primary-800">
                    <span class="text-yellow-300 text-sm font-bold">
                        {{ strtoupper(substr(Auth::user()->name ?? (Auth::user()->username ?? 'O'), 0, 1)) }}
                    </span>
                </div>
            </div>
        </header>
